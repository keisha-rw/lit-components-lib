// This file is a copy of the element-internals-polyfill
// with a minor modification (lines 10 - 13) so it does not
// run in a non-browser environment. Replace with actual module
// when it supports SSR
//
// See: https://github.com/calebdwilliams/element-internals-polyfill/issues/108
//
// Lines 296-299 (in wireSubmitLogic) were also changed so we don't try to use :is
// selectors in jsdom
// See: https://github.com/jsdom/jsdom/issues/3432
(function () {
  'use strict';
    ...
    mutationList.forEach((mutationRecord) => {
      ...
      added.forEach((node) => {
        ...
        if (validityUpgradeMap.has(node)) {
          ...
          node.setAttribute(
            'internals-valid',
            internals.validity.valid.toString()
          );
          node.setAttribute(
            'internals-invalid',
            (!internals.validity.valid).toString()
  ...
  const wireSubmitLogic = (form) => {
    const SUBMIT_BUTTON_SELECTOR = typeof window !== 'undefined' &&
    window.navigator &&
  ...
  const reconcileValidity = (validityObject, newState, form) => {
    ...
    Object.keys(newState).forEach(
      (key) => (validityObject[key] = newState[key])
    );
    if (form) {
      setFormValidity(form);
    }
  ...
  class HTMLFormControlsCollection {
    constructor(elements) {
      ...
      __classPrivateFieldSet(
        this,
        _HTMLFormControlsCollection_elements,
        elements,
        'f'
      );
    ...
    [((_HTMLFormControlsCollection_elements = new WeakMap()),
    Symbol.iterator)]() {
      return __classPrivateFieldGet(
        this,
        _HTMLFormControlsCollection_elements,
  ...
  function isElementInternalsSupported() {
    ...
    // MODIFIED THIS LINE TO GET RID OF RANDOMNESS
    const randomName = `element-internals-feature-detection-${new Date()
      .getTime()
      .toString(36)
      .replace(/[^a-z]+/g, '')}`;
